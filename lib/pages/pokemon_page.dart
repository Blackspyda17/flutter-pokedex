...
class _PokemonPageState extends State<PokemonPage> {
  @override
  void initState() {
    context.read<PokemonDetailsBloc>().add(GetPokemonDetailsEvent(url: widget.url));
    super.initState();
  }
                              ...
                              IconButton(
                                onPressed: () {
                                  if (pokemon.id != 1) {
                                    ...
                                    context.read<PokemonDetailsBloc>().add(GetPokemonDetailsEvent(url: previousUrl));
                                  }
                                },
                                icon: (pokemon.id != 1)
                                    ? SvgPicture.asset(
                              ...
                              BlocBuilder<PokemonListBloc, PokemonListState>(
                                builder: (context, state) {
                                  if (state is PokemonListSuccess) {
                                    return IconButton(
                                      onPressed: () {
                                        if (pokemon.id != state.pokemonList.count) {
                                          final previousUrl = '$baseUrl/pokemon/${pokemon.id + 1}';
                                          context.read<PokemonDetailsBloc>().add(GetPokemonDetailsEvent(url: previousUrl));
                                        }
                                      },
                                      icon: (pokemon.id != state.pokemonList.count)
                                          ? SvgPicture.asset(
